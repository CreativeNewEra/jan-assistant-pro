...
class ConfigSchema:
    ...
    def _validate_field(
        self, field_path: str, value: Any, rule: ValidationRule
    ) -> tuple[bool, str]:
        ...
        if not isinstance(value, rule.field_type):
            return False, (
                f"Field '{field_path}' must be of type "
        ...
        if rule.min_value is not None and value < rule.min_value:
            return False, (
                f"Field '{field_path}' must be >= {rule.min_value}, got {value}"
            )

        if rule.max_value is not None and value > rule.max_value:
            return False, (
                f"Field '{field_path}' must be <= {rule.max_value}, got {value}"
            )

        # Allowed values validation
        if rule.allowed_values is not None and value not in rule.allowed_values:
            return False, (
                f"Field '{field_path}' must be one of "
        ...
        if rule.pattern is not None and isinstance(value, str):
            ...
            if not re.match(rule.pattern, value):
                return False, (
                    f"Field '{field_path}' does not match "
        ...
        if rule.validator_func is not None:
            try:
                ...
                return False, (f"Field '{field_path}' validation failed: {str(e)}")
...
def create_jan_assistant_schema() -> ConfigSchema:
    ...
    schema.add_rule(
        "api.api_key",
        ValidationRule(
            field_type=str, required=True, description="API authentication key"
        ),
    )
    ...
    schema.add_rule(
        "api.model",
        ValidationRule(
            field_type=str, required=True, description="Model name to use for API calls"
        ),
    )
    ...
    schema.add_rule(
        "memory.file",
        ValidationRule(
            field_type=str, required=True, description="Path to memory storage file"
        ),
    )
    ...
    schema.add_rule(
        ...
        ValidationRule(
            ...
            default=128,
            min_value=0,
            description="In-memory API cache size",
        ),
    )
    schema.add_rule(
        "cache.api.ttl",
        ValidationRule(
            ...
            default=300,
            min_value=0,
            description="In-memory API cache TTL in seconds",
        ),
    )
    schema.add_rule(
        "cache.disk.dir",
